(function () {
  ...
  function getSelectedSpaceExtras() {
    return selectedSpaceExtraIds
      .map((id) => SPACE_EXTRAS[id])
      .filter(Boolean);
  }

  function getSpaceExtrasTotal() {
    return getSelectedSpaceExtras().reduce((sum, extra) => sum + extra.price, 0);
  }

  function getSpaceExtrasCount() {
    return selectedSpaceExtraIds.length;
  }
  ...
  function updateSummary() {
    ...
    const spaceExtrasCount = getSpaceExtrasCount();
    ...
    if (els.summarySpaceExtrasList) {
      els.summarySpaceExtrasList.innerHTML = getSelectedSpaceExtras()
        .map((extra) => {
  ...
  function updateSpaceExtrasFooter() {
    const count = getSpaceExtrasCount();
  ...
  function updateSpaceExtrasUI() {
    ...
    const extras = getSelectedSpaceExtras();

    document.querySelectorAll(".cbs-extra-option").forEach((btn) => {
      btn.classList.toggle(
        "selected",
        selectedSpaceExtraIds.includes(btn.dataset.extraId),
      );
    });
  ...
  function removeSelectedSpaceExtra(id) {
    selectedSpaceExtraIds = selectedSpaceExtraIds.filter(
      (extraId) => extraId !== id,
    );
    updateSpaceExtrasUI();
  }

  function toggleSpaceExtra(id) {
    if (!id || !SPACE_EXTRAS[id]) return;
    if (selectedSpaceExtraIds.includes(id)) {
      selectedSpaceExtraIds = selectedSpaceExtraIds.filter(
        (extraId) => extraId !== id,
      );
    } else {
      selectedSpaceExtraIds = selectedSpaceExtraIds.concat(id);
    }
    updateSpaceExtrasUI();
  }
  ...
  function setupSpaceExtras() {
    document.querySelectorAll(".cbs-extra-option").forEach((btn) => {
      btn.addEventListener("click", () => {
        toggleSpaceExtra(btn.dataset.extraId);
      });
    });
    document
      ...
      ?.addEventListener("click", (e) => {
        ...
        if (!btn) return;
        e.stopPropagation();
        removeSelectedSpaceExtra(btn.getAttribute("data-remove-extra"));
      });
  ...
  function populateReview() {
    ...
    const spaceExtras = getSelectedSpaceExtras();
    const allAddons = [...groomerAddons, ...spaceExtras];
