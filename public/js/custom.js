...
function initCustomSelects() {
    // Clean up old event listeners by cloning elements first
...
// Initialize custom selects on load and after navigation
...
// Initialize testimonial carousel on load and after navigation
if (document.readyState === "loading") {
...
// Load JSON from data/pet-breeds.json using Laravel asset base URL
async function loadPetBreedsData() {
    // Only load once
    if (window._petBreedsLoaded) return;

    // Get base URL from meta tag (for subdirectory deployments)
    ...
    try {
        ...
        window._petBreedsLoaded = true;
    } catch (error) {
        console.error("Failed to load pet breeds:", error);
    }
}

// Initialize pet breed data on load and after navigation
if (document.readyState === "loading") {
