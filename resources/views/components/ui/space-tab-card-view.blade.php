...
===== Usage Example =====
<x-space-tab-card-view
    :spaces="$spaces"
    :showLoadMore="true"
    loadMoreText="Show More Spaces"
    loadMoreUrl="{{ route('spaces.index') }}?page=2"
/>
...
'rating' => 4.3,
'reviews_count' => 20,
'experience' => 'Professional grooming spaces in London. Spotless, well-equipped, easy to access.',
'amenities' => 'Bath • Table • Dryer ...',
'slots' => ['Mon 1, 08:30 AM', 'Wed 27, 09:15 AM'],
'price' => 38,
'time_frame' => '/ hour',
'image' => asset('assets/images/space_card1.png'),
'tags' => ['Salons'],
'is_top_rated' => true,
'is_favorite' => false,
'is_locked' => false,
...
'loadMoreUrl' => '#', // URL for load more (if not using Alpine/Livewire)
])
...
<section class="section">
    <div class="row g-3">
                ...
                <div class="right">
                    <div class="top-row">
                        <div class="tags" aria-hidden="true">
                            @foreach($space['tags'] ?? ['Salons'] as $tag)
                            <div class="tag">{{ $tag }}</div>
                            @endforeach
                        </div>
                        <div class="icons" aria-hidden="true">
                            @if($space['is_top_rated'] ?? false)
                    ...
                    <h2 class="name">{{ $space['name'] ?? 'Space Name' }}</h2>
                    <p class="hosted-by">Hosted by <span>{{ $space['hosted_by'] ?? 'Host' }}</span></p>
                    ...
                    <div class="studio-details mt-1 d-flex flex-column justify-content-between mt-2">
                        <div class="meta d-flex align-items-center justify-content-between">
                            ...
                            <div class="rating" title="Rating">
                                ...
                                <span>{{ $space['rating'] ?? '0.0' }} <small style="color:var(--muted)">({{ $space['reviews_count'] ?? 0 }})</small></span>
                            </div>
                        </div>
                    </div>

                    <p class="experience">{{ $space['experience'] ?? 'Professional grooming spaces in London. Spotless, well-equipped, easy to access.' }}</p>

                    <p class="amenities d-flex">
                        <b>Amenities:</b>&nbsp;{{ $space['amenities'] ?? 'Bath • Table • Dryer ...' }}
                    </p>
                    ...
                    <div class="slots-price d-flex flex-column">
                        ...
                        <div class="price">From <span>£{{ $space['price'] ?? '0' }}</span> <span class="time-frame">{{ $space['time_frame'] ?? '/ hour' }}</span></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($showLoadMore && count($spaces) > 0)
