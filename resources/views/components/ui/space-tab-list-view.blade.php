...
@props([
'spaces' => [], // Array or Collection of space objects
'showLoadMore' => false,
'loadMoreUrl' => '#',
])
...
<div class="row">
    ...
    <div class="col-lg-10">
        ...
        @php
            ...
            $patternId = 'space-list-pattern-'.uniqid('', false).'-'.$index;
            ...
            $slots = array_slice((array) ($space['slots'] ?? []), 0, 2);
            $amenities = $space['amenities'] ?? 'Bath • Table • Dryer ...';
            if (is_array($amenities)) {
                $amenities = implode(' • ', $amenities);
            }
            $priceUnit = $space['price_unit'] ?? $space['time_frame'] ?? $space['price_period'] ?? 'hour';
            $priceUnit = ltrim((string) $priceUnit);
            if ($priceUnit !== '' && ! str_starts_with($priceUnit, '/')) {
                $priceUnit = '/ '.$priceUnit;
            }
        @endphp
        <div class="card space-cards-list-view mt-3">
            {{-- Left: notched image so the shield sits in the cutout --}}
            <div class="left">
            ...
            <div class="right">
                ...
                <div class="studio-details mt-1 d-flex align-items-center justify-content-between">
                    ...
                    <div class="meta d-flex align-items-center justify-content-between">
                        ...
                        <div class="rating" title="Rating">
                            ...
                            <span>{{ $space['rating'] ?? '4.3' }} <small style="color:var(--muted)">({{ $space['reviews_count'] ?? $space['reviews'] ?? 20 }})</small></span>
                        </div>
                    </div>
                </div>

                <p class="list-view-desc">{{ $space['description'] ?? $space['experience'] ?? '' }}</p>
                ...
                <div class="slots-price d-flex align-items-center">
                    ...
                    <div class="price">From <span>£{{ $space['price'] ?? 0 }}</span> <span class="time-frame">{{ $priceUnit }}</span></div>
                </div>
            </div>
        </div>
        @endforeach
        ...
        <div class="col-lg-12 text-center section-gap">
            <button class="load-more" onclick="window.location.href='{{ $loadMoreUrl }}'">Load More</button>
        </div>
        @endif
    </div>
    <div class="col-lg-1"></div>
