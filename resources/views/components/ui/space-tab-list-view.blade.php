...
<div class="row">
    ...
    <div class="col-lg-10">
        ...
        <div class="card space-cards-list-view mt-3">
            <!-- Left side: Image + top-left SVG badge -->
            <div class="left">
                ...
                <img src="{{ $space->image_url ?? 'https://placehold.co/312x229' }}" alt="{{ $space['name'] }}" width="312" height="229">
            </div>
            ...
            <div class="right">
                <div class="top-row">
                    <div class="tags" aria-hidden="true">
                        <div class="tag">{{ $space->tag ?? 'Salons' }}</div>
                    </div>
                ...
                <div class="studio-details mt-1 d-flex align-items-center justify-content-between">
                    <div class="studio">
                        ...
                        <p class="hosted-by">Hosted by <span>{{ $space['host_name'] ?? '' }}</span></p>
                    </div>
                    <div class="meta d-flex align-items-center justify-content-between">
                        ...
                        <div class="rating" title="Rating">
                            ...
                            <span>{{ $space['rating'] ?? '' }} <small style="color:var(--muted)">({{ $space['reviews_count'] ?? '' }})</small></span>
                        </div>
                    </div>
                </div>

                <p class="list-view-desc">{{ $space['description'] ?? '' }}</p>

                <p class="amenities d-flex">
                    <b>Amenities:</b>&nbsp;
                    {{
        is_array($space['amenities'] ?? null)
        ? implode(' &#8226; ', $space['amenities'])
        : ($space['amenities'] ?? '')
    }}
                </p>
                ...
                <div class="slots-price d-flex align-items-center">
                    <div class="slots d-flex" aria-label="Available slots">
                        @foreach(array_slice($space->slots ?? [], 0, 2) as $slot)
                        <div class="slot {{ $loop->first ? 'highlight' : '' }}">{{ $slot }}</div>
                        @endforeach
                    </div>
                    <div class="price">From <span>{{ $space->price }}</span> <span class="time-frame">/ {{ $space->price_period ?? 'hour' }}</span></div>
                </div>
            </div>
        </div>
        @endforeach
