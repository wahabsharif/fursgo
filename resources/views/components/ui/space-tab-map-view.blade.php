...
<div {{ $attributes->merge(['class' => 'section-gap']) }}>
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="row">
                ...
                @php
                    ...
                    $distance = $space['distance'] ?? '1.0 mi';
                    if (is_numeric($distance)) {
                        $distance = $distance.' mi';
                    }
                    $host = $space['host'] ?? $space['hosted_by'] ?? $space['host_name'] ?? 'Dev É.';
                    $reviews = $space['reviews'] ?? $space['reviews_count'] ?? 20;
                    $experience = $space['experience'] ?? $space['description'] ?? 'Professional grooming spaces in London. Spotless, well-equipped, easy to access.';
                    $amenities = $space['amenities'] ?? 'Bath • Table • Dryer ...';
                    if (is_array($amenities)) {
                        $amenities = implode(' • ', $amenities);
                    } elseif (is_string($amenities) && ! str_contains($amenities, '•')) {
                        $parts = array_map('trim', explode(',', $amenities));
                        $amenities = implode(' • ', $parts).(count($parts) ? ' ...' : '');
                    }
                    $tags = $space['tags'] ?? ['Salon'];
                    if (is_string($tags)) {
                    ...
                    $topRated = $space['top_rated'] ?? $space['is_top_rated'] ?? true;
                    $price = $space['price'] ?? 30;
                    $priceUnit = $space['price_unit'] ?? $space['time_frame'] ?? '/ hour';
                    $patternId = 'space-map-pattern-'.uniqid('', false).'-'.$index;
                @endphp
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 255 130" fill="none">
                                <defs>
                                    <pattern id="{{ $patternId }}" patternUnits="userSpaceOnUse" width="255" height="130">
                                        <image href="{{ $imageUrl }}" width="255" height="130" preserveAspectRatio="xMidYMid slice" />
                                    </pattern>
                                </defs>
                        ...
                        <div class="right">
                            <div class="top-row">
                                <div class="tags" aria-hidden="true">
                                    @foreach($tags as $tag)
                                    <div class="tag">{{ is_string($tag) ? trim($tag) : $tag }}</div>
                                    @endforeach
                                </div>
                                <div class="icons" aria-hidden="true">
                                    @if($topRated)
                                    <button type="button" class="first-icon" title="Top-rated">
                                    ...
                                    @endif
                                    <button type="button" class="second-icon" title="Favourite">
                                    ...
                                    <button type="button" class="third-icon" title="lock">
                            ...
                            <p class="hosted-by">Hosted by <span>{{ $host }}</span></p>
                            ...
                            <div class="studio-details mt-1 d-flex flex-column justify-content-between mt-2">
                                <div class="meta d-flex align-items-center justify-content-between">
                                    ...
                                    <div class="rating" title="Rating">
                                        ...
                                        <span>{{ $space['rating'] ?? '4.3' }} <small style="color:var(--muted)">({{ $reviews }})</small></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="slots-price d-flex flex-column">
                                ...
                                <div class="price">From <span>£{{ $price }}</span> <span class="time-frame">{{ $priceUnit }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {{-- Right Column: Map (initialized by customer_journey.js) --}}
