@php
    $tone = $this->verificationStatusTone();
    $isApproved = $tone === 'approved';
    $isRejected = $tone === 'rejected';

    $statusLabel = $isApproved ? 'Approved' : ($isRejected ? 'Failed' : 'More Info Needed');
    $copyLine = $isApproved
        ? 'Your verification has been approved.'
        : ($isRejected
            ? 'We were unable to verify your information or documents.'
            : 'We need a bit more information to complete your verification.');
    $copyMuted = $isApproved
        ? 'You can now set up payouts and access all provider features!'
        : ($isRejected
...
<div class="verification-card verification-status-card verification-status-card--{{ $tone }}"
    ...
    @else
        <div class="verification-status-visual verification-status-visual--{{ $tone }}" aria-hidden="true">
            <img class="verification-status-visual__shield" src="{{ asset($shield) }}" alt="">
            <img class="verification-status-visual__mark" src="{{ asset($mark) }}" alt="">
        </div>
    @endif
    <h2 class="verification-approved-heading">Verification Status</h2>
    <p class="verification-approved-status">{{ $statusLabel }}</p>
    <div class="verification-approved-copy">
        <p>{{ $copyLine }}</p>
        <span class="verification-approved-copy-muted">{{ $copyMuted }}</span>
    </div>
    <div class="verification-approved-actions" style="justify-content: center;">
        @if ($isApproved)
            <x-common.button label="Build Your Profile" width="auto" bg-color="#B2CE78" wire:click="continueToBuildProfile"
                box-shadow="0 5px 8px 0 rgba(0, 0, 0, 0.10)" style="min-width: 179px; padding: 0 1.75rem;" />
        @else
            <x-common.button label="Review Submission" width="226px" bg-color="#3B3731" wire:click="reviewSubmission"
