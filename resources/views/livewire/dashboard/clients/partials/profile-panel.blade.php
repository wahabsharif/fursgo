...
<section class="client-profile-wrapper {{ $isSpaceUser ? 'is-space-user' : '' }}" aria-label="Client profile">
            ...
            <div class="client-profile-stats">
                <button type="button" class="client-profile-stat" wire:click="setProfileTab('upcoming')">
                    ...
                    <span class="client-profile-stat-value"><span
                            class="client-profile-stat-figure">{{ $meta['upcoming_count'] }}</span> / bookings</span>
                ...
                <button type="button" class="client-profile-stat" wire:click="setProfileTab('bookings')">
                    ...
                    <span class="client-profile-stat-value"><span
                            class="client-profile-stat-figure">{{ $meta['completed_count'] }}</span> / bookings</span>
                    <span class="client-profile-stat-chevron" aria-hidden="true">›</span>
                </button>
    ...
    <div class="client-profile-toolbar">
        ...
        @if (in_array($profileActiveTab, ['upcoming', 'bookings', 'payments'], true))
            <div class="client-profile-sort" x-data="{
                open: false,
                menuLeft: 0,
                menuTop: 0,
                menuWidth: 220,
    ...
    @if ($profileActiveTab === 'pets')
        <div class="client-profile-pets-grid">
            @forelse ($this->profilePets as $pet)
                <article class="client-profile-pet-card" wire:key="client-pet-{{ $pet->id }}">
                    <div class="client-profile-pet-avatar">
                        @if ($pet->photo)
                    ...
                    <div>
                        <div class="client-profile-pet-name">{{ $pet->name ?: '—' }}</div>
                        @if (trim((string) ($pet->pet_type ?? '')) !== '')
                            <div class="client-profile-pet-type">{{ $pet->pet_type }}</div>
                        @endif
                    </div>
                </article>
            @empty
                <p class="client-profile-empty">No pets found.</p>
            @endforelse
        </div>
        ...
        <div class="client-profile-table-shell">
            <table class="client-profile-table">
                ...
                <tbody>
                            ...
                            @if ($isSpaceUser)
                                ...
                                <td>
                                    <div class="client-profile-booking-details">
                                        <div>{{ $bookingDate }}</div>
                                        <div class="client-profile-booking-time-space">{{ $bookingTime }}</div>
                                    </div>
                                </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $isSpaceUser ? 6 : 7 }}" class="client-profile-empty-cell">No bookings
                                found.</td>
                        </tr>
                    @endforelse
                </tbody>
...
<style>
    ...
    .client-profile-pets-grid {
        display: grid;
        grid-template-columns: repeat(auto-fill, minmax(220px, 1fr));
        gap: 1rem;
    }

    .client-profile-pet-card {
        display: flex;
        align-items: center;
        gap: 0.85rem;
        border: 1px solid #E6E6E6;
    ...
    .client-profile-pet-avatar img {
        width: 100%;
        height: 100%;
        object-fit: cover;
    }

    .client-profile-pet-name {
        color: #3B3731;
        font-family: Lato;
        font-size: 16px;
        font-weight: 600;
