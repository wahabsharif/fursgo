{{--
  Specific groomer/space unavailable: apology copy, selected card, upcoming slots.
  Props: $lead, $other, $title, $kind (groomer|space), $selected (optional model/array)
--}}
@php
    $kind = $kind ?? 'groomer';
    $selected = is_array($selected ?? null)
        ? $selected
        : (isset($selected) && method_exists($selected, 'toArray') ? $selected->toArray() : (array) ($selected ?? []));

    $isSpace = $kind === 'space';

    $name = $selected['name'] ?? ($isSpace ? 'The Garden Grooming Spot' : 'Sarah W.');
    $host = $selected['host'] ?? $selected['hosted_by'] ?? $selected['host_name'] ?? 'Chloe D.';
    $studio = $selected['studio_name'] ?? ($isSpace ? null : 'Sarah’s Grooming Studio');
    $distance = $selected['distance'] ?? '1.0';
    if (is_numeric($distance)) {
        $distance = $distance.' mi';
    } elseif (! str_contains((string) $distance, 'mi')) {
        $distance = $distance.' mi';
    }
    $rating = $selected['rating'] ?? '4.3';
    $reviews = $selected['reviews_count'] ?? $selected['reviews'] ?? 20;
    $price = $selected['price'] ?? 30;
    $priceUnit = $selected['price_unit'] ?? $selected['time_frame'] ?? ($isSpace ? '/ hour' : '');
    $priceUnit = ltrim((string) $priceUnit);
    if ($isSpace && $priceUnit !== '' && ! str_starts_with($priceUnit, '/')) {
        $priceUnit = '/ '.$priceUnit;
    }

    $experience = $selected['experience'] ?? $selected['experience_text'] ?? $selected['description']
        ?? ($isSpace
            ? 'Outdoor garden grooming area. Calm, spacious, and ideal for stress-free sessions in fresh air.'
            : 'Hi, I’m Sarah — a professional groomer in West London, specialising in small breeds, anxious pets, & gentle handling! Gentle, breed-specific trims · 6+ years experience.');

    $amenities = $selected['amenities'] ?? 'Bath • Table • Dryer ...';
    if (is_array($amenities)) {
        $amenities = implode(' • ', $amenities);
    }

    $tags = $selected['tags'] ?? ($isSpace ? ['Salon'] : ['Home Visit', 'Mobile Station']);
    if (is_string($tags)) {
        $decoded = json_decode($tags, true);
        $tags = is_array($decoded) ? $decoded : array_map('trim', explode(',', $tags));
    }
    if (! is_array($tags) || count($tags) === 0) {
        $tags = $isSpace ? ['Salon'] : ['Home Visit', 'Mobile Station'];
    }

    $imagePath = $selected['image'] ?? $selected['image_url'] ?? ($isSpace ? 'images/space_card3.png' : 'images/card1.png');
    if (is_string($imagePath) && str_starts_with($imagePath, 'http')) {
        $parsed = parse_url($imagePath, PHP_URL_PATH);
        $imageUrl = $parsed ?: $imagePath;
    } elseif (str_starts_with((string) $imagePath, '/')) {
        $imageUrl = $imagePath;
    } else {
        $imageUrl = asset(str_replace('assets/', '', (string) $imagePath));
    ...
    $patternId = 'specific-'.($kind).'-'.substr(md5($imageUrl.$name), 0, 8);

    $upcomingSlots = $selected['upcoming_slots'] ?? [
        'Monday 11, 12:30 PM',
        'Tuesday 12, 12:30 PM',
        'Wednesday 25, 12:30 PM',
        'Sunday 30, 12:30 PM',
        'Tuesday 12, 12:30 PM',
        'Wednesday 25, 12:30 PM',
        'Sunday 30, 12:30 PM',
...
<div class="col-lg-12 d-flex flex-column align-items-center justify-content-center mt-5 mb-5">
    <p class="not-found-message">{!! $lead !!}</p>
    <p class="other-available-text">{{ $other }}</p>
</div>
...
<section class="section-gap">
    <div class="row">
        ...
        <div class="col-lg-10">
            @if ($isSpace)
            {{-- Space selected card: rectangular notched image (custom specific_space list) --}}
            <div class="card space-cards-list-view specific mt-5">
                <div class="left">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="312" height="229" viewBox="0 0 312 229" fill="none">
                        <defs>
                            <pattern id="{{ $patternId }}" patternUnits="userSpaceOnUse" width="312" height="229">
                                <image href="{{ $imageUrl }}" width="312" height="229" preserveAspectRatio="xMidYMid slice" />
                            </pattern>
                        </defs>
                        <path d="M312 224C312 226.761 309.761 229 307 229H5.00001C2.23858 229 0 226.761 0 224V37.1406C0 34.3792 2.23858 32.1406 5 32.1406H27C29.7614 32.1406 32 29.902 32 27.1406V5C32 2.23858 34.2386 0 37 0H307C309.761 0 312 2.23858 312 5V224Z" fill="url(#{{ $patternId }})" />
                    </svg>
                </div>
                <div class="right">
                    <div class="top-row">
                    ...
                    <div class="studio-details mt-1 d-flex align-items-center justify-content-between">
                        ...
                        <div class="meta d-flex align-items-center justify-content-between">
                            ...
                            <div class="rating" title="Rating">
                                ...
                                <span>{{ $rating }} <small style="color:var(--muted)">({{ $reviews }})</small></span>
                            </div>
                        </div>
                    </div>

                    <p class="list-view-desc">{{ $experience }}</p>
            ...
            @else
            {{-- Groomer selected card: circular avatar --}}
            <div class="card specific mt-5">
                <div class="left">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="138" height="138" viewBox="0 0 138 138">
                        <defs>
                            <pattern id="{{ $patternId }}" patternUnits="userSpaceOnUse" width="138" height="138">
                                <image href="{{ $imageUrl }}" xlink:href="{{ $imageUrl }}" width="138" height="138" preserveAspectRatio="xMidYMid slice" />
                            </pattern>
                        </defs>
                        <circle cx="69" cy="69" r="69" fill="url(#{{ $patternId }})" />
                    </svg>
                </div>
                <div class="right">
                    <div class="top-row">
        ...
        <div class="col-lg-10">
            <div class="row">
                ...
                @foreach($upcomingSlots as $slotIndex => $slot)
                <div class="col-lg-3 mt-3">
                    <div class="slots-timing {{ $slotIndex === 0 ? 'active' : '' }}">
                        <p>{{ is_string($slot) ? trim($slot) : $slot }}</p>
                    </div>
                </div>
                @endforeach
            </div>
